"""Compile gates for the LorentzNet-LGATrSlim hybrid pair (docs/cgenn-compile.md).

These two nets were born compile-friendly: dense top-k kNN (`idx`/`neighbor_mask`, no
data-dependent `nonzero`), 2-operand einsums only, no cached properties, no tensor-valued
repeats — so like Stage 2 this is gate-running unless the gates say otherwise.

Fixtures (recorded at pre-gate HEAD, record-before-edit discipline):
    LNSLIM_HYBRID_COMPILE=record python -m pytest tests/experiments/test_lorentznet_hybrid_compile.py -q
...
a production-regime sweep — every sweep shape keeps max padded length > quick-config
knn_k = 4, since dense topk(k) requires P >= k and small-P regimes would be a legitimate
extra compile, as in the CGENN hybrids).
"""
...
FIX = REPO / "tests" / "fixtures" / "lorentznet_hybrid_compile"
RECORD = os.environ.get("LNSLIM_HYBRID_COMPILE") == "record"
RUN_COMPILE_GATES = os.environ.get("CGENN_COMPILE_GATES") == "1"

MODELS = ["tag_LorentzNetLGATrSlimGraphTrans", "tag_LorentzNetLGATrSlimGraphGPS"]
...
def test_bit_eager_vs_fixtures(model, prec):
    ...
    if RECORD:
        ...
        return
    if not path.exists():
        pytest.skip("no lorentznet_hybrid_compile fixtures recorded")
